...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = 
    w.toLowerCase.groupBy(c => c).map { case (c, str) => (c, str.length) }.toList.sorted
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val yMap = y.toMap
    x.map { case (c, count) => (c, count - yMap.getOrElse(c, 0)) }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def anagramsRec(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
      else {
        for {
          combo <- combinations(occurrences)
          word <- dictionaryByOccurrences.getOrElse(combo, List())
          rest <- anagramsRec(subtract(occurrences, combo))
        } yield word :: rest
      }
    }
